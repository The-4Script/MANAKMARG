...
def test_transcription_prefers_the_accurate_model_and_sends_the_domain_prompt(keyed, monkeypatch):
    ok = FakeGroq([FakeResponse(200, {"text": "Is hallmarking mandatory in Jaipur?", "segments": []})])
    monkeypatch.setattr(groq.requests, "post", ok)
    assert groq.transcribe_audio(b"x" * 2048, "voice.webm", "audio/webm", keyed) == "Is hallmarking mandatory in Jaipur?"
    assert ok.calls[0]["data"]["model"] == "whisper-large-v3"  # accuracy first; turbo is only the fallback
    assert "BIS" in ok.calls[0]["data"]["prompt"]


def test_hallucinated_silence_is_treated_as_no_speech(keyed, monkeypatch):
    # Every segment reads as silence by the standard no_speech_prob/avg_logprob heuristic on both the primary and
    # the fallback model, so the caller should see "no speech" rather than the invented text.
    silent_segment = [{"no_speech_prob": 0.95, "avg_logprob": -1.5}]
    hallucinated = FakeGroq(
        [
            FakeResponse(200, {"text": "thank you for watching", "segments": silent_segment}),
            FakeResponse(200, {"text": "thank you for watching", "segments": silent_segment}),
        ]
    )
    monkeypatch.setattr(groq.requests, "post", hallucinated)
    assert groq.transcribe_audio(b"x" * 2048, "voice.webm", "audio/webm", keyed) == ""
    assert len(hallucinated.calls) == 2  # both models tried before giving up


def test_looping_phrase_is_treated_as_no_speech(keyed, monkeypatch):
    looping = FakeGroq(
        [
            FakeResponse(200, {"text": "IS 2062 IS 2062 IS 2062 IS 2062 IS 2062", "segments": []}),
            FakeResponse(200, {"text": "IS 2062 IS 2062 IS 2062 IS 2062 IS 2062", "segments": []}),
        ]
    )
    monkeypatch.setattr(groq.requests, "post", looping)
    assert groq.transcribe_audio(b"x" * 2048, "voice.webm", "audio/webm", keyed) == ""


def test_hallucination_on_primary_model_falls_through_to_a_good_fallback(keyed, monkeypatch):
    silent_segment = [{"no_speech_prob": 0.95, "avg_logprob": -1.5}]
    recovered = FakeGroq(
        [
            FakeResponse(200, {"text": "hmm hmm hmm hmm hmm", "segments": silent_segment}),
            FakeResponse(200, {"text": "Which tests are required for IS 2062?", "segments": []}),
        ]
    )
    monkeypatch.setattr(groq.requests, "post", recovered)
    assert groq.transcribe_audio(b"x" * 2048, "voice.webm", "audio/webm", keyed) == "Which tests are required for IS 2062?"
