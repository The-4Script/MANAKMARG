...
_SOURCES = (
    ...
    SourceDef(
        ...
        ingestion_method="Excel export from the Published Standards List page (Export button; heading 'Total'). The "
        "supplied data/1.xlsx is the initial snapshot; the weekly refresh (manakmarg.refresh) downloads it again, "
        "validates it and parses it with openpyxl by manakmarg.ingest.excel_standards.",
        authority="official_primary",
        access_status="ok",
        document_type="xlsx",
        copyright_notes=STANDARD_EXPORT_RIGHTS,
        access_notes="Title cell 'Total'. 24,014 rows in the supplied snapshot; later row counts and generation times are "
        "in data/refresh/refresh_log.jsonl. No per-row Department or Group column.",
    ...
    SourceDef(
        ...
        ingestion_method="One Excel export per ministry: the Ministry-wise page lists the ministries, each opens its "
        "Published Standards List page, and that page's Export produces the file; the title cell names the "
        "classification node. Collected weekly by manakmarg.refresh and parsed by manakmarg.ingest.excel_standards.",
        authority="official_primary",
        access_status="partial",
        document_type="xlsx",
        copyright_notes=STANDARD_EXPORT_RIGHTS,
        access_notes="The supplied snapshot covers 18 nodes alphabetically from 'Department of Atomic Energy' to "
        "'Ministry of Communications - Department of Telecommunications (DOT)'. The Ministry-wise page's own aggregate "
        "download is offered only to signed-in users with an approved role and is not used; the weekly refresh "
        "collects every ministry individually and records coverage in data/refresh/refresh_log.jsonl. Ministry is "
        "classification context only, never a basis for compulsory certification.",
    ...
    SourceDef(
        ...
        document_type="json",
        copyright_notes=STANDARD_EXPORT_RIGHTS,
        access_notes="Undocumented API behind an Angular app that blocks developer tools; not used for Department or "
        "Group classification, which therefore stays empty. The weekly standards refresh makes only the anonymous "
        "export and ministry-list requests the public Published Standards pages make (docs/DATA_REFRESH.md); "
        "sign-in-only downloads are not used.",
    ),
