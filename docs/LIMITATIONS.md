...
* Transcription uses `whisper-large-v3` first (lower word-error-rate than the turbo model, particularly on
  Hindi/English code-switched speech), falling back to `whisper-large-v3-turbo` only if the first call fails, and
  sends a short domain-vocabulary prompt (BIS/standards terms, common city names) to bias Whisper's spelling — this
  cannot invent words that were not spoken, only reduce mishearing of known terms. A result that reads as Whisper
  inventing text over silence or noise (very low confidence throughout, or the same short phrase repeated on a
  loop) is treated as no speech rather than shown as an answer.
